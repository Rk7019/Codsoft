...
writer.color(PRIMARY_COLOR) # Set the initial color
...
def display_message(message, y_pos=50, font=PROMPT_FONT, color=PRIMARY_COLOR):
    """Clears previous messages and displays a new one."""
    writer.clear() # Clear previous text written by this turtle
    writer.color(color) # Use the specified color (defaults to PRIMARY_COLOR)
    writer.goto(0, y_pos)
    writer.write(message, align="center", font=font)
    writer.color(PRIMARY_COLOR) # Reset to the primary color after writing
...
while length is None:
    ...
    if length_str is None: # User pressed Cancel
        display_message("Operation cancelled by user.", 0, font=ERROR_FONT, color=CANCEL_COLOR)
        time.sleep(2)
...
if length and length > 0:
    ...
    if length == 0: # Specifically check for our cancellation signal
         ...
         display_message("Password generation cancelled.", 0, color=CANCEL_COLOR) # Re-display if needed
    else: # Should ideally not happen with current logic, but good practice
        display_message("Could not generate password.", 0, font=ERROR_FONT, color=ERROR_COLOR)


# --- Keep Window Open ---
writer.goto(0,-100)
writer.color(PRIMARY_COLOR) # Ensure exit message uses the primary color
